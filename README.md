...
6. Recreate exact conda environment (use environment_history.yml instead if there is an error, which can happen on different OS)
...
8. Install code as python package into environment

    ```bash
    make package
    ```

9. Create BLAST databases (Needs >100GB of space and takes several hours, pre-computed pssms are availabe in data/intermediate)

    ```bash
    make blast_databases
    ```

10. Run the notebooks in order, according to their filenames
